...
export def open-workspace-picker [] {
    if ($env.HERDR_ENV? | default "") != "1" {
        print -e "Workspace picker requires a Herdr-managed pane."
        return
    }

    let lookup = (^herdr workspace list | complete)
    if $lookup.exit_code != 0 {
        let reason = $lookup.stderr | str trim
        print -e (
            if ($reason | is-empty) { "Cannot list open Herdr workspaces." } else { $reason }
        )
        return
    }
    ...
    let workspaces = (
        try {
            $lookup.stdout | from json | get result.workspaces
        } catch {
            print -e "Herdr returned invalid workspace data."
            return
        }
    )
    if ($workspaces | is-empty) {
        print "No open Herdr workspaces."
        return
    }

    let labels = (
        $workspaces
        | each {|workspace| try { $workspace.label } catch { "" } }
    )
    let rows = (
        $workspaces
        | each {|workspace|
            let workspace_id = try { $workspace.workspace_id } catch { "" }
            let label = try { $workspace.label } catch { $workspace_id }
            if ($workspace_id | is-empty) {
                null
            } else if (($labels | where $it == $label | length) > 1) {
                let suffix = try {
                    $workspace.worktree.checkout_path | path basename
                } catch {
                    $workspace_id
                }
                {id: $workspace_id, display: $"($label)  •  ($suffix)"}
            } else {
                {id: $workspace_id, display: $label}
            }
        }
        | where {|row| $row != null}
    )
    ...
    let input = $rows | each {|row| [$row.id $row.display] | str join $delimiter } | str join "\n"
    let result = (
        $input
        | ^fzf
            --delimiter $delimiter
            ...
            --layout reverse-list
            --prompt "workspace> "
            --header "Focus open Herdr workspace"
            --footer "Enter focus  •  Esc cancel"
            --color "fg:-1,bg:-1,fg+:-1,bg+:-1,hl:cyan,hl+:cyan,pointer:magenta,prompt:cyan,footer:yellow"
        | complete
    )
    if $result.exit_code != 0 {
        return
    ...
    let workspace_id = $selected | split row $delimiter | first
    let focused = (^herdr workspace focus $workspace_id | complete)
    if $focused.exit_code != 0 {
        let reason = $focused.stderr | str trim
        print -e (
            if ($reason | is-empty) { "Herdr could not focus the selected workspace." } else { $reason }
        )
    }
}
